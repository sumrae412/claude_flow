...
5. **If Tier 1 has HIGH+ findings:** Continue cascade normally — partition remaining reviewers by `always` vs `conditional` (file_patterns match), group by `cascade_tier`, run Tier 2+ in parallel within their tier
