...
| **UI mockup (round-trip)** | `docs/design/<feature>/mockups/<screen>.excalidraw` | Yes — user edits, Phase 4 re-reads, Phase 5 consumes | Phase 4 → user → Phase 5 |

`<feature>` is a kebab-case slug of the feature name (e.g. `settings-page`). One file per screen/component for mockups; one file total for the architecture diagram.
