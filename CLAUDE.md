...
- `PR_REVIEWER_REVALIDATE=1` — opt-in second-pass FP filter (`src/revalidate.ts`). After dedup, asks the model to re-read each finding against the diff and emit `TRUE_POSITIVE` / `FALSE_POSITIVE` / `UNCERTAIN`; drops false positives, keeps the rest. Inspired by [vercel-labs/deepsec](https://github.com/vercel-labs/deepsec)'s `revalidate` stage (they report ~50% FP-rate reduction on whole-repo scans). Doubles cost across the findings list — don't default on. Most useful when primary is high-recall/low-precision (NVIDIA ensemble). Empirical FP-cut on small PR diffs is unverified — A/B against your repo before relying on it.
- `PR_REVIEWER_MAX_REVALIDATE` — Rule-6 budget cap on the revalidation pass (default 30, `0` disables). Findings are sorted by severity and only the top-N are revalidated; the cheap-to-lose tail is kept (never dropped) and surfaced as `unverified` in the PR comment's partial-coverage banner. Bounds the "doubles cost" blast radius when a high-recall ensemble produces a long findings list.
